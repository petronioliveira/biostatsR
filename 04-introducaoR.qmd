...
O *RStudio* é o ambiente de desenvolvimento integrado mais utilizado com o R. Ele serve para facilitar a escrita, execução e depuração de código R, bem como para gerenciar projetos, visualizar dados e criar gráficos. O *RStudio* é um membro ativo da comunidade R. Foi fundado em 2009 por Joseph J. Allaire, engenheiro de software americano. O *RStudio*, inspirado pelas inovações dos usuários de R em ciência, educação e indústria, desenvolveu ferramentas gratuitas e abertas para facilitar o uso do R. O *RStudio* é escrito em linguagem C++ e foi inicialmente focado apenas na linguagem R. Com o tempo, desenvolveu suporte para *Python* e *VSCode*. Em 2022, para acompanhar essa mudança, foi anunciada a mudança do nome da empresa que o desenvolve para [Posit](https://posit.co/) e, em 2024, lançado um novo IDE multilíngue denominado *Positron* [@blog_2024]. O *Positron* é construído sobre o *VS Code* e oferece suporte nativo tanto ao R quanto ao Python em um único ambiente. Embora já seja bastante funcional e continue em desenvolvimento ativo [@positron2025], o *RStudio* permanece a escolha principal para usuários focados exclusivamente em R, em razão de suas ferramentas específicas para essa linguagem.

#### Instalação do *R Studio*

Para instalar o *RStudio*, acessar o [**site**](https://posit.co/download/rstudio-desktop/) e clicar em *Download* para obter a versão desejada. Recomenda-se a versão *RStudio Desktop* – *Open Source License* que é gratuita. Esta versão entrega as ferramentas integradas para o R.
...
Em sequência, executar o instalador baixado *RStudio-2026.05.0-218.exe* [^04-introducaor-2] e seguir as suas instruções.

[^04-introducaor-2]: Disponível em junho de 2026

#### Iniciando o *RStudio*

Para iniciar o *RStudio* basta clicar no ícone indicativo (@fig-logo) que se encontra no menu *Iniciar* do Windows.

![Logotipo do RStudio](images/rstudio_logotipo.png){#fig-logo fig-align="center" width="3cm"}
...
![Tela inicial do RStudio](images/rstudio_tela-inicial.png){#fig-inicial fig-align="center" width="16.7cm"}
...
![Realizando operações direto no Console](images/console.png){#fig-console fig-align="center" width="15.4cm"}
...
![Bloco de Notas do RStudio: R Script](images/r_script.png){#fig-script fig-align="center" width="16.7cm"}

Um diferencial do *RStudio* é que os comandos são autocompletáveis (@fig-autocompletavel). Basta começar a escrever o comando, inserindo 3 ou mais caracteres, por exemplo, `summ` referente a função `summary ()`, usada para sumarizar um conjunto de dados, e surge um menu de opções, facilitando a digitação.

![Menu autocompletável](images/menu_autocompletavel.png){#fig-autocompletavel fig-align="center" width="17cm"}

Após digitar no *Console*, para que seja executado o comando há necessidade de clicar na tecla *Enter*; no *RScript*, clicar em *Run*, acima, na barra, no lado direito, ou usar o atalho *Ctrl + Enter* (@fig-script). Textos podem ser copiados e colados no script e linhas em branco podem ser inseridas. Além disso, no final da sua sessão, é possível salvar o arquivo, que poderá ser recarregado no futuro, se precisar refazer a análise.

Os *scripts* do *RStudio* são apenas arquivos de texto com a extensão (`.R`). Quando se cria um *R Script*, aparece como *Sem título* (*Untitled*) (@fig-autocompletavel). Antes de começar a digitar um novo *script* no *RStudio*, recomenda-se salvar o atual com um novo nome de arquivo. Dessa forma, se algo no computador falhar durante o trabalho, o código ficará protegido.

Além da digitação de comandos, o *R Script* permite fazer comentários onde tudo que for escrito, após o símbolo $\#$ (@fig-script), não é considerado, é apenas uma explicação, um esclarecimento. Os comentários são literais, escritos diretamente para explicar o comando executado. São repetidos na saída do *Console* sem aparecer nos resultados.
...
![Tutoriais do RStudio](images/tutoriais.png){#fig-tutorial fig-align="center" width="12cm"}
...
![Ajuda no RStudio](images/ajuda.png){#fig-ajuda fig-align="center" width="17cm"}
...
![Instalação do pacote ‘ggplot2’ usando a caixa de diálogo ‘Install Packages’](images/instalaacao_pacotes.png){#fig-install fig-align="center" width="11.5cm"}
...
Além da função `p_load()`, o pacote `pacman` tem outras funções, entre elas a função `p_update()` que atualiza o pacote e, se usada sem especificar o pacote, atualiza todos. Para saber mais sobre o pacote `pacman`, use a ajuda.
...
![Diretório de Trabalho](images/working_directory.png){#fig-wd fig-align="center" width="17.4cm"}
...
![Assistente de novo projeto](images/novo_projeto.png){#fig-projeto fig-align="center" width="11.5cm"}
...
O R permite salvar valores dentro de um *objeto*. Os objetos são criados utilizando o *operador de atribuição* (`<-`). Para digitar este operador, basta teclar o sinal *menor que* (`<`), seguido de *hífen* (`-`), sem espaços. Existe um atalho que é pressionar `Alt + -`. O sinal `=` pode ser usado no lugar do operador `<-`. No entanto, este é preferível para atribuição de variáveis em R por motivos estruturais. Técnicas de escopo [^04-introducaor-4] impedem que = crie variáveis persistentes quando utilizado dentro de funções [@r_base_syntax], além de ferir as diretrizes contemporâneas de legibilidade do ecossistema [@tidyverse_assignment_style].

[^04-introducaor-4]: Técnicas de escopo (ou regras de escopo) são as diretrizes que determinam onde e quando uma variável existe e pode ser acessada dentro de um código de programação. Elas definem os limites de visibilidade de um objeto para que o sistema saiba exatamente a qual valor se está referindo.
...
Os dados inseridos nos objetos pertencem as seguintes classes [^04-introducaor-5]:

[^04-introducaor-5]: Atributos de um objeto.

1.  [Caracteres (character)]{.underline}: são categóricos ou qualitativos. Em geral, são caracteres ou texto. São escritos entre aspas simples ou duplas e não são modificáveis.

2.  [Numérico (numeric)]{.underline}: são números inteiros (`integer`) ou decimais ou ponto flutuante (`dbl` = `double`). A sintaxe para o número inteiro é 42L, 3L[^04-introducaor-6], pois se fosse escrito como 42, o R automaticamente o trata como um número flutuante (`double`).
...
[^04-introducaor-6]: O `L` vem da linguagem C, onde `long` é um tipo inteiro. É uma convenção que o R herdou.

É a partir do conhecimento do tipo de classe que as funções sabem o que exatamente fazer com um objeto. Por exemplo, não é possível somar duas letras e se for feita a tentativa de somar "a" e "b", o R retorna um erro: [Error in "a" + "b": non-numeric argument to binary operator]{style="color:red"}.
...
O resultado deste comando aparecerá na aba *Help*, na parte inferior, à direita (@fig-ajuda).
...
### Definindo suas próprias funções {#sec-funcpropria}
...
![Anatomia de uma função](images/funcao_anatomia.png){#fig-anatomia fig-align="center" width="15cm"}
...
O resultado da função foi atribuído a um objeto, `CV`, e usada a função `round()`, nativa do R, para limitar os decimais em apenas um dígito. Caso contrário, o resultado teria 5 dígitos, desnecessários, pois não modifica a informação.
...
Neste momento, não serão discutidos os códigos da função. Ela será utilizada como uma função qualquer com os dados do arquivo `dadosNotas.xlsx` [^04-introducaor-7]. Os argumentos da função são o dataframe (`df = dados`), a variável x (`var.x = sexo`) e a variável y (`var.y = notas`). Dessa forma, está se comparando a nota dos alunos de Bioestatística por sexo:

[^04-introducaor-7]: Para maiores detalhes, consulte o @sec-testet.
...
#| label: fig-twonotas
#| fig-align: 'center'
#| fig-cap: "Comparação das notas de Bioestatística por sexo"
#| crop: false
...
plotBpT(var.x = sexo, var.y = notas, df = dados)
```

Observe na @fig-twonotas os boxplots, eles têm posições bem diferentes (veja @sec-bxp), sugerindo uma diferença nos escores de notas entre homens e mulheres e o resultado do teste *t* ($p = 0,001$) confirma isso. Existe uma diferença estatisticamente significativa (veja também, @sec-testet).

#### Ativação de uma função criada

Para ativar uma função previamente criada, usa-se a função nativa `source ()`. O argumento desta função é o caminho onde se encontra a função buscada, por exemplo, a função `imc()` criada acima [^04-introducaor-8]:

[^04-introducaor-8]: Este caminho é o do computador do autor.

```{r}
#| eval: false
source("funcoes/imc.R")
```

Outra maneira de ativar uma função é usar a função `here` do pacote `here`. É mais robusto porque não depende do diretório de trabalho atual. É útil quando o código é executado a partir de um script em subpasta:

```{r}
source(here::here("funcoes", "imc.R"))
```
...
Como não há nomes no objeto `n.filhos`, a saída da função `names(n.filhos)` é `NULL`.
...
Como é numérico, teoricamente, seria possível realizar operações matemáticas com ele. Entretanto, sabe-se que os números 1 e 2, representam categorias e, consequentemente, devem ser transformados em fatores, usando a função `factor()`:
