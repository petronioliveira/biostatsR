# Medidas de ocorrência de doença {#sec-ocorrencia}
...
![Incidência](images/incidencia.png){#fig-incidencia fig-align="center" width="13cm" height="4.7cm"}
...
![Pessoas-tempo (estudo epidemiológico hipotético)](images/incidencia_1.png){#fig-incidencia1 fig-align="center" width="10cm"}
...
Para testar essa hipótese, foi identificada uma coorte de 1.567 pessoas totalmente cegas e 13.292 sujeitos com deficiência visual grave. As informações sobre a incidência de câncer foram obtidas do Registro Sueco de Câncer [@feychting1998reduced]. Foram diagnosticados 136 casos de câncer em 22050 pessoas-ano em risco totalmente cegas e 1709 casos de câncer em 127650 pessoas-anos em risco com deficiência visual grave.
...
As saídas mostram que para cada 1000 pessoas cegas (a função foi multiplicada por 1000) acompanhadas por um ano, ocorreram 6,2 (IC~95%~: 5,2 – 7,3) casos de câncer. Uma taxa de incidência, praticamente, metade da taxa de incidências das pessoas com deficiência visual grave. Os IC~95%~ não são coincidentes, o que significa que essa diferença é significativa. Houve, na amostra, uma incidência menor de câncer entre os indivíduos totalmente cegos, sugerindo que a melatonina possa ser um fator protetor contra o câncer.
...
dados <- data.frame(
  Exposição  = c("Expostos", "Não Expostos", "Total"),
  Doença     = c("a", "c", "a + c"),
  Sem_Doença = c("b", "d", "b + d"),
  Total      = c("a + b", "c + d", "a + b + c + d")
)

# Construção da tabela com destaques
tabela <- flextable(dados) |>
  set_header_labels(
    Exposição = "Exposição",
    Doença = "Com Doença",
    Sem_Doença = "Sem Doença",
    Total = "Total"
  ) |>
  # Alinhamento
  ...
  bg(part = "header", bg = "#2B4C7E") |>               # Fundo azul escuro
  color(part = "header", color = "white") |>            # Texto em branco
  bold(part = "header", bold = TRUE) |>                 # Texto em negrito
  
  # --- DESTAQUE DA LINHA DO TOTAL ---
  bg(i = 3, bg = "#F0F4F8") |>                          # Fundo cinza/azul claro na linha 3
  bold(i = 3, bold = TRUE) |>                           # Negrito na linha do Total
  
  # --- AJUSTE DE BORDAS E LARGURA ---
  hline(i = 2, border = fp_border(color = "black", width = 1.5)) |> # Linha separando o Total
  autofit()
...
Usando a @tbl-tab_2x2, a fórmula $odds =\frac{p}{1 -p}$ e que
...
A partir da tabela de contingência $2 \times 2$ ( @tbl-tab_2x2), tem-se que o estimador do RR é dado por:
...
Em 18 de abril de 1940, ocorreu um surto de gastroenterite, após um jantar, em uma igreja (@fig-oswego), na vila de Lycoming, Condado de Oswego, Nova York. Das 80 pessoas que compareceram ao jantar, 46 das 75 entrevistadas desenvolveram posteriormente doença gastrointestinal aguda. Devido à sua natureza direta, o incidente tornou-se um estudo de caso de ensino padrão para gerações de epidemiologistas.

![Igreja de Oswego, 1940](images/oswego.png){#fig-oswego fig-align="center" width="8cm"}
:::
...
Tomando como base a estrutura da tabela de contingência 2 x 2 , @tbl-tab_2x2, tem-se:
...
A partir da variável `idade_mae`, criar a variável `faixaEtaria`, dividindo as parturientes em menores de 20 anos (adolescentes) e ≥ 20 anos. Para isso, usou-se a função `cut()` do pacote base. Revise os argumentos desta função.
...
A variável `inf_cong` encontra-se como uma variável numérica e deve ser transformada em fator:
...
Após estes procedimentos, constrói-se uma tabela $2 \times 2$:
...
Utilizando a tabela de contingência $2 \times 2$ ( @tbl-tab_2x2), o RA fica expresso da seguinte maneira:
...
dados <- data.frame(
  Grupo   = c("Aspirina", "Placebo"),
  IAM     = c(139, 239),
  Sem_IAM = c(10898, 10795),
  Total   = c(11037, 11034)
)
...
tab <- colformat_num(
  tab,
  big.mark = ".",   # separador de milhares
  decimal.mark = "," # separador decimal
)

# Ajustes visuais
tab <- theme_vanilla(tab)
tab <- align(tab, align = "center", part = "all")
tab <- autofit(tab)
...
No exemplo do *Physicians' Health Study* @steering1989final, o RAR é igual a:
...
round(c(li_RAR, RAR, ls_RAR), 4)
```

Portanto, a Redução Absoluta do Risco foi igual a 0,0091 (IC~95%~: 0,0056-0,0125). A partir destes resultados, pode-se calcular o intervalo de confiança para o NNT:
...
Para o cálculo do NND, usa-se a função `risks()`, como mencionado antes:
