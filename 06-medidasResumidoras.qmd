...
matriz <- matrix(dados$peso_rn, nrow = 5,ncol = 6)
...
## Dados usados neste capítulo {#sec-dados-cap6}

Para as demonstrações práticas, será usado o banco de dados `dadosMater.xlsx` (ver @sec-dadosMater). Após carregá-lo, serão filtrados os partos a termo e selecionadas as variáveis necessárias (`idade_mae`, `anos_est`, `peso_rn`, `apgar1`). Por último, será extraída com a função `slice_sample()` do pacote `dplyr` (@sec-dplyr) uma amostra de n = 200. Cada vez que este comando for reproduzido, retornará uma nova série de 200 valores, diferente da anterior. Para tornar o código reproduzível, retornando o mesmo conjunto de valores, deve-se usar uma “semente” (`seed`), usando a função `set.seed()`, cujo argumento é um número que identificará a série gerada. Após extrair a amostra, esta será atribuída a um objeto denominado, `dados`:
...
1.  Ordenar o conjunto de dados, por exemplo, a variável `dados$peso_rn`, usando a função `sort()`:
...
- No exemplo, tem-se 200 valores. Para transformar a amostra em uma amostra com *n* ímpar, remover aleatoriamente uma observação dos valores dos `dados$peso_rn`:
...
- Remover o `r index_aleatorio`º valor (index_aleatório) da lista de `dados$peso_rn` que corresponde ao peso selecionado:

```{r}
pesos_restantes <- dados$peso_rn[-index_aleatorio]
```
...
```{r}
median(dados$peso_rn, na.rm=TRUE)
...
```{r}
quantile (dados$peso_rn, c (0.25, 0.50, 0.75))
```

Observe que o percentil 50º é igual a mediana. O percentil 75º é o ponto do conjunto de dados onde 75% dos recém-nascidos têm um peso inferior a `r quantile (dados$peso_rn, 0.75)`g e 25% está acima deste valor.
...
```{r}
round(mean (dados$peso_rn, na.rm = TRUE, trim = 0.20), 1)
```
...
```{r}
range (dados$peso_rn, na.rm = TRUE)
```
...
```{r}
quantile (dados$anos_est, c(0.25,0.75))
```

```{r}
summary(dados$anos_est)
```

Portanto, o IIQ está entre `r quantile (dados$anos_est, 0.25)` a `r quantile (dados$anos_est, 0.75)` anos de estudo ou, `r quantile (dados$anos_est, 0.75) - quantile (dados$anos_est, 0.25)` anos de estudos completos. Em outras palavras, 50% das mulheres desta amostra têm entre `r quantile (dados$anos_est, 0.25)` a `r quantile (dados$anos_est, 0.75)` anos de estudo.
...
#| message: false
#| fig-align: 'center'
#| fig-cap: "Dispersão dos dados em torno da média."
#| crop: false
curve (dnorm (x, 
              mean=0, 
              sd=0.7), 
...
Foi criada uma função específica para isso, já multiplicada por 100 (ver @sec-funcpropria). Será repetida aqui a função para facilitar a visualização:
...
```{r}
round(coefVar(dados$peso_rn), 1)
```

Usando outra variável do banco de dados, por exemplo, `dados$idade_mae`, o CV será igual a:

```{r}
round(coefVar(dados$idade_mae), 1)
```

O peso do recém-nascido tem um CV = `r round(coefVar(dados$peso_rn), 1)` % e a idade materna um CV = `r round(coefVar(dados$idade_mae), 1)` %, mostrando que esta tem uma maior variabilidade. Quanto menor o desvio padrão, menor o CV e, consequentemente, menor a variabilidade. Um CV $\ge$ 50%, sugere que a variável tem uma distribuição assimétrica.
