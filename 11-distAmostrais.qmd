...
## Dados para este capítulo

Os dados deste capítulo são provenientes do conjunto de dados `dadosMater.xlsx` (@sec-dadosMater), considerando apenas as variáveis `altura`, `renda` e `fumo`:

```{r}
dados <- read_excel("dados/dadosMater.xlsx") |> 
  dplyr::select(altura, renda, fumo) |> 
...
A função `slice_sample()` do pacote `dplyr`extrairá uma amostra [^11-distamostrais-1] de n = 30 da população. As funções `mean()` e `sd()` calcularão a média e o desvio padrão, respectivamente:
...
Média e desvio padrão das 5000 amostras:
...
Amostras diferentes selecionadas da mesma população darão resultados diferentes porque contêm elementos diferentes. Isso é evidente nas médias das amostra1 e amostra2, `r round(mean(amostra1$altura, na.rm =TRUE), 3)`m e `r round(mean(amostra2$altura, na.rm =TRUE), 3)`m, respectivamente, comparadas com a média da população igual a `r round(mean(dados$altura, na.rm =TRUE), 3)`m .
...
![Erro Padrão versus Desvio Padrão](images/erro_desvio_padrao.png){#fig-standard fig-align="center" width="12cm"}
...
resumo <- dados |> 
  select (renda) |> 
  dplyr::summarise (media = mean (dados$renda, na.rm = TRUE),
                    dp = sd(dados$renda, na.rm = TRUE),
                    mediana = median(dados$renda, na.rm = TRUE),
                    assimetria = e1071::skewness(dados$renda),
                    curtose = e1071::kurtosis(dados$renda))
resumo
```

O desvio padrão é grande em relação à média, com um coeficiente de variação de `r round(resumo$dp/resumo$media, 3)*100`% e uma mediana < média. Estas métricas junto com os coeficientes de assimetria e curtose apontam para a assimetria positiva da variável `renda`. O gráfico da @fig-skewpos confirma esta afirmação:
...
Os valores da média e do desvio padrão calculados para a distribuição de probabilidade dessa população fornecem os valores dos parâmetros populacionais $\mu$ e $\sigma$. Esses valores são $\mu$ =`r round(resumo$media, 2)` *sm* [^11-distamostrais-3] e $\sigma$ =`r round(resumo$dp, 2)` *sm*.\
...
[Média e desvio padrão das 1000 amostras]{.underline}
...
Ou seja, extraindo-se 1000 amostras de n = 30 e calculando as mesmas métricas anteriores, observa-se que, embora a distribuição populacional original seja assimétrica, a distribuição amostral da média se aproxima bastante da distribuição gaussiana (@fig-semskew) (veja também @sec-rnorm).

::: callout-important
## Importância do Teorema Central do Limite

O TCL é um dos pilares da inferência estatística [@massad2004inferencia]. Entender a sua importância é como destrancar a porta para aplicar testes, construir intervalos de confiança e fazer previsões com segurança, mesmo quando os dados parecem confusos.
