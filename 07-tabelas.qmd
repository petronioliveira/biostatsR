...
2.  **Cabeçalho** - Identifica as variáveis apresentadas. Inclui unidades de medida quando necessário (ex: idade em anos, glicemia em mg/dL)
...
![Elementos de uma tabela](images/tabela1.png){#fig-tabela1 fig-align="center" width="16cm"}
...
4.  **Tomada de decisão**: Permitem que se tire conclusões rápidas sobre um conjunto de dados. Por exemplo, na @tbl-droga, verifica-se que "a maioria das gestantes, no Hospital Geral de Caxias do Sul, referem não ser drogaditas" ou "72% das gestantes, no Hospital Geral de Caxias do Sul, referem não ser drogaditas".
...
A frequência relativa pode ser expressa como uma fração, decimal ou, mais comumente, como uma porcentagem (*frp*). É útil para comparar distribuições de dados com tamanhos totais diferentes. Por exemplo, na @tbl-droga, 2,13% das parturientes declararam ser alcoolistas.\
...
#### Dados para os exemplos {#sec-dados-cap7}

Para os exemplos, o dataframe `dadosMater.xlsx` será acionado para fornecer dados, semelhante ao realizado na @sec-dados-cap6.
...
A função `prop.table()` é usada para calcular as frequências relativas a partir de uma tabela de frequência absoluta. Em outras palavras, ela transforma as contagens em proporções ou porcentagens. A função `round()` recebe a função `prop.table()` para arredondar para três dígitos a frequência relativa (`fr`):
...
Embora as funções `table()` e `prop.table()` sejam separadas, é muito comum usá-las juntas para construir uma **tabela de frequência completa**. Portanto, os passos para a construção de uma tabela de frequência no R são os seguintes
...
Para análises mais avançadas ou para variáveis quantitativas contínuas, pode-se usar a função `mutate()` , junto com a função `case_when()` [@geeks2025case_when] , usada para criar a variáveis `categIdade` e `estNutri`, na @sec-dados-cap7. Também é possível associar a função `mutate()` com a função `cut()`, como se verá a seguir.
...
Como exemplo prático, a variável de interesse será representada pelos pesos dos recém-nascidos (`peso_rn`) do conjunto de dados, `dados`, mencionado na (@sec-dados-cap7).
...
Para a variável `dados$peso_rn`, como existem 200 observações, pode-se, de acordo com a @tbl-sturges, usar ao redor de 9 classes. O R tem uma função `nclass.Sturges ()`, que também pode ser usada:
...
A fórmula é apenas a diferença absoluta dos valores mínimos e máximo , calculada pela função `diff()` [^07-tabelas-3], dividida pelo número de classes (*k*), arredondado com a função `round()`.
...
A partir desses dados, é possível construir as classes: O limite inferior da primeira classe é `r  min(dados$peso_rn)` e o limite superior é `r min(dados$peso_rn) + h`, exclusive; o limite inferior da segunda classe inclui o valor de `r min(dados$peso_rn) + h` e vai até `r min(dados$peso_rn) + (2* h)`, excluindo-o, pois ele será limite inferior da terceira classe e assim por diante
...
2.  Tendo os pontos de corte estabelecidos, basta agora criar a variável `pesoCateg`, ou seja a variável `peso_rn` numérica classificada em 9 classes:
...
tab_full$Frp <- cumsum(tab_full$frp)
```

**Passo 4**: Adicionar os nomes das categorias como uma coluna

```{r}
tab_full$classePeso <- rownames (f)
```
...
**Passo 6**: Reorganizar as colunas e exibir a tabela
...
Assim, pode-se dizer que a prevalência de baixo peso na maternidade do Hospital Geral de Caxias do Sul é bem mais alta que a prevalência no Brasil que é de 6,1% (IC95%: 4,5-8,3%) [@viana2013birth].
...
O pacote `gt` é um pacote que permite a apresentação de tabelas de maneira limpa e organizada [@iannone2020gt]. Funciona através do `tidyverse` com os comandos *pipe*, `|>` (nativo) ou `%>%` (necessita do `tidyverse`), o que facilita sua aplicação.
...
Para maiores detalhes consulte [Create beautiful tables with gt](https://gt.rstudio.com/) ou [Introduction to Creating gt Tables](https://gt.rstudio.com/articles/gt.html).
...
2.  [Cabeçalho da tabela]{.underline}: a função `tab_header()` adiciona um título e subtítulo à tabela.
    ...
    - Aplicado estilo ao título principal com a função `tab_style()` - negrito (`bold`), tamanho médio, fonte arial e cor cinza escura (`gray18`)
...
6.  Por último se incluiu a fonte e uma nota explicativa.
...
**Passo 1**: Da mesma forma como nas tabelas criadas pelo pacote `gt`, o `flextable` também necessita de tabela base (dataframe ou tibble) [@navarro2024flextable]. Será usada a mesma tabela (`tab_full`). Uma tabela simples (@tbl-ft1) pode ser facilmente gerada:
...
A @tbl-ft5 já está ótima, mas outros temas podem ser usados como `theme_booktabs()` (padrão), `theme_vader()`, `theme_apa()`, `theme_zebra()`, `theme_box()`, `theme_tron_legacy ()`.
...
```{r}
#| label: tbl-ft8
#| tbl-cap: "Tabela com seleção especial de células"
...
### Pacote gtsummary {#sec-gtsummary}
...
Para maiores detalhes consulte a [vinheta](https://www.danieldsjoberg.com/gtsummary/articles/tbl_summary.html) da função.

#### Dados para o exemplo

Como exemplo, será usado um dataframe originário do conjunto de dados `dadosMater.xlsx` (veja @sec-dados-cap7), contento uma amostra de 200 observações com as seguintes variáveis relacionadas aos recém-nascidos:
...
A @tbl-baixopeso mostra como construir uma tabela de comparação binária. O objetivo não é fazer um estudo sobre causas de baixo peso ao nascer, mas apenas mostrar os recurso de construção com este excelente pacote.
